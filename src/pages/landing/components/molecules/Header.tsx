...
function Header() {
  ...
  return (
    <>
      <Container as="header" className={`flex justify-between items-center transition-all duration-200 ${scrolled ? "sticky top-4 z-[1000] bg-[#1d20315d] shadow-md pl-2 px-8 py-2 rounded-full backdrop-blur-2xl" : ""}`}>
        <img src={scrolled ? Logo : LogoDark} width={50} height={50} alt='logo de capo' color='text-primary' className='p-1' />
        <div className="hidden lg:flex items-center justify-end gap-6 py-4">
          {MENU.map((element) => (
            <Typography
              key={element.text}
              variant="h6"
              className={` hover:text-primary-2 cursor-pointer font-normal transition-all duration-200 hover:font-bold ${scrolled ? "text-white" : "text-primary"
                }`}
            >
